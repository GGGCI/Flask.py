...
def get_response():
    ...
    print("msg: ",message)
    if action == 1:
        result = subprocess.run(['/home/ubuntu/AWS_Trend_Hakethon/myenv/bin/python3', '/home/ubuntu/AWS_Trend_Hakethon/src/flask_to_lambda/flask_to_lambda.py', message, 'Summary_GenAI'], capture_output=True, text=True)
        ...
        result = subprocess.run(['/home/ubuntu/AWS_Trend_Hakethon/myenv/bin/python3', '/home/ubuntu/AWS_Trend_Hakethon/src/flask_to_lambda/flask_to_lambda.py', message, 'Cybersecurity_GenAI'], capture_output=True, text=True)
        response = result.stdout
        if result.returncode == 0 and not result.stderr:
            response = result.stdout
        else:
